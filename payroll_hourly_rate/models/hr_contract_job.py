...
class hr_contract_job(models.Model):
    ...
    def _get_current_hourly_rate(self):
        ...
        for contract_job in self:
            ...
            if (
                contract_job.hourly_rate_class_id
                and contract.salary_computation_method == "hourly"
            ):
                rate_class = contract_job.hourly_rate_class_id
                rates = [
                    r
                    for r in rate_class.line_ids
                    if (
                        r.date_start <= today
                        and (not r.date_end or today <= r.date_end)
                    )
                ]
    ...
    hourly_rate_class_id = fields.Many2one(
        "hr.hourly.rate.class", string="Hourly Rate Class"
    )
    hourly_rate = fields.Float(
        string="Hourly Rate", compute="_get_current_hourly_rate"
    )
