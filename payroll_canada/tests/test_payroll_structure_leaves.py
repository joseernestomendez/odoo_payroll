...
class TestPayrollStructureLeaves(TestPayrollStructureBase):
    ...
    def setUp(self):
        ...
        self.vac_accrual_id = employee.get_leave_accrual(
            self.ref("payroll_activity.holiday_status_vacation")
        ).id
        ...
        holidays_public_ids = self.public_holidays_model.search(
            cr,
            uid,
            [("year", "=", 2015), ("country_id", "=", self.canada_id)],
            context=context,
        )
    ...
    def test_leaves_in_payslip_worked_days(self):
        ...
        self.assertEqual(
            round(payslip_2["VAC_REQ"], 2), round((4 * 8 + 20) * 40, 2)
        )
        ...
        self.assertEqual(
            ...
            round((80.0 + 70.0) * 40.0 / 20),
            2,
        )

        self.assertEqual(
            round(payslip_2["PUBLIC_AVAIL"], 2), round(70.0 * 2 * 40.0 / 20, 2)
        )

        self.assertEqual(
            round(payslip_2["PUBLIC_REQ"], 2), round((8 + 8) * 40, 2)
        )

        self.assertEqual(payslip_2["PUBLIC_TAKEN"], payslip_2["PUBLIC_AVAIL"])
        ...
        self.assertEqual(
            payslip_2["GROSSP"],
            payslip_2["COMP_TAKEN"]
            + payslip_2["VAC_TAKEN"]
            + payslip_2["SL_TAKEN_CASH"]
    ...
    def test_leaves_in_payslip_input(self):
        ...
        self.assertEqual(
            payslip_2["VAC_UNUSED"], 1500 - 500 + 2000 - payslip_2["VAC_TAKEN"]
        )
