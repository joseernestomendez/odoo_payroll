...
XML_SCHEMA_PATH = (
    "%s/../data/cra_xml_schemas/layout-topologie.xsd" % CURRENT_DIR
)
...
class TestHrCraT4Summary(TestHrCraT4Base):
    ...
    def set_t4_amount(self, slip, box_ref, amount):
        ...
        box_id = self.data_model.get_object_reference(
            self.cr, self.uid, "payroll_canada", box_ref
        )[1]
    ...
    def test_compute_t4_summary(self):
        ...
        self.payslips = [
            self.get_payslip_lines(slip_id)
            for slip_id in [
                self.payslip_ids[1],
                self.payslip_ids[2],
                self.payslip_ids[3],
        ...
        totals = {t.box_id.xml_tag: t.amount for t in summary.total_ids}
        ...
        self.assertEqual(totals["tot_padj_amt"], 500 + 1500)
