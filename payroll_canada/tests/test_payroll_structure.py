...
class TestCanadaPayrollStructure(TestPayrollStructureBase):
    ...
    def test_create_payslip_low_wage(self):
        ...
        self.assertEqual(
            payslip["FIT_I"], round(payslip["GROSSP"] * (1 + 0.008 + 0.018), 2)
        )
        ...
        self.assertEqual(
            payslip["FIT_F"], 15 + pension_rate * payslip["GROSSP"]
        )
    ...
    def test_create_payslip_higher_wage(self):
        ...
        self.assertEqual(
            ...
            round(
                payslip["FIT_I"] * p
                - (pension_rate * payslip["GROSSP"] + 15 + 16 + 17 + 18 + 19)
                * p
            ),
        )
    ...
    def test_create_payslip_end_of_year(self):
        ...
        # RRP
        self.assertEqual(payslip4["RPP_EE_C"], payslip4["GROSSP"] * 0.001)

        # the k2 tax credit is based on ei and cpp contributions
        self.assertEqual(payslip4["FIT_K2"], 913.68 + 2425.50)

        self.payslip_model.write(
            cr, uid, payslips.values(), {"state": "draft"}, context=context
        )
