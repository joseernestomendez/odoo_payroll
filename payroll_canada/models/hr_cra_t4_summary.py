...
class HrCraT4Summary(models.Model):
    ...
    def generate_slips(self):
        ...
        for employee in employees:
            ...
            slip = next((s for s in slips if s.employee_id == employee), False)
    ...
    def generate_xml(self):
        ...
        for t4_slip in self.t4_slip_ids:
            ...
            std_amounts = t4_slip.amount_ids.filtered(
                lambda a: not a.is_other_amount and a.box_id.appears_on_summary
            )

            amounts_dict = {
                a.box_id.xml_tag: "%.2f" % a.amount for a in std_amounts
            }

            other_amounts = t4_slip.amount_ids.filtered(
                lambda a: a.is_other_amount and a.box_id.appears_on_summary
            )
            ...
            t4_slip_dict = {
                ...
                "T4_AMT": amounts_dict,
                "OTH_INFO": {
                    a.box_id.xml_tag: "%0.2f" % a.amount for a in other_amounts
                },
            }
            ...
            if t4_slip.rpp_dpsp_rgst_nbr:
                t4_slip_dict["rpp_dpsp_rgst_nbr"] = t4_slip.rpp_dpsp_rgst_nbr
        ...
        # The company address
        company_address_dict = cra_summary_obj.make_address_dict(company)
        ...
        if self.contact_extension:
            contact_dict["cntc_extn_nbr"] = self.contact_extension
        ...
        # distinct structure.
        t619_xml = cra_summary_obj.make_t619_xml(slip_return_xml, self)
