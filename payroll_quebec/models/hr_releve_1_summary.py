...
class HrReleve1Summary(models.Model):
    ...
    def get_payslips(self):
        ...
        year = int(self.year)
        date_from = datetime(year, 1, 1).strftime(DEFAULT_SERVER_DATE_FORMAT)
        date_to = datetime(year, 12, 31).strftime(DEFAULT_SERVER_DATE_FORMAT)
    ...
    def _compute_cnt_amount_payable(self):
        for s in self:
            s.cnt_amount_payable = s.cnt_salaries * s.cnt_rate / 100
    ...
    def _compute_wsdrf_expenses_available(self):
        for s in self:
            s.wsdrf_expenses_available = (
                s.wsdrf_previous_reported + s.wsdrf_expenses_current
            )
    ...
    qpp_amount_er = fields.Float(
        "Employer Contributions", required=True, **FLOAT_PARAM
    )
    ...
    qit_amount_2 = fields.Float(
        "Relevés 2", help="Relevés 2 (box J)", **FLOAT_PARAM
    )
    ...
    hsf_salaries = fields.Float(
        "Salaries Eligible", required=True, **FLOAT_PARAM
    )
    ...
    hsf_exemption_amount = fields.Float("Amount Exempted", **FLOAT_PARAM)
    ...
    hsf_reduction_basis = fields.Float(
        "Amount Admissible for the Reduction", **FLOAT_PARAM
    )
    hsf_reduction_rate = fields.Float("Reduction Rate", **FLOAT_PARAM)
    ...
    hsf_amount_remitted = fields.Float(
        "Contributions Remitted",
        required=True,
        help="The HSF contributions already remitted during " "the year.",
        **FLOAT_PARAM
    )
    ...
    cnt_salaries = fields.Float(
        "CNT - Salaries Eligible", required=True, **FLOAT_PARAM
    )
    ...
    wsdrf_expenses = fields.Float(
        "Expenses Deduced",
        required=True,
        help="The amount of expenses " "used to reduce the contribution",
        **FLOAT_PARAM
    )
    wsdrf_reported = fields.Float(
        "Expenses Reported",
        compute="_compute_wsdrf_reported",
        help="The amount of expenses to be " "reported to further years.",
        **FLOAT_PARAM
    )

    wsdrf_contribution = fields.Float(
        "Contribution", compute="_compute_wsdrf_contribution", **FUNCTION_PARAM
    )

    total_balance = fields.Float(
        "Total Balance", compute="_compute_total_balance", **FUNCTION_PARAM
    )
    total_receivable = fields.Float(
        "Total Receivable", compute="_compute_total_balance", **FUNCTION_PARAM
    )
    ...
    def generate_slips(self):
        ...
        for employee in employees:
            ...
            slip = next((s for s in slips if s.employee_id == employee), None)
            ...
            for slip in summary.releve_1_ids:
                if slip.state in ["draft", "cancelled"]:
                    raise ValidationError(
                        _("Error"),
                        _(
                            "Every Relevé 1 must be confirmed before sending "
