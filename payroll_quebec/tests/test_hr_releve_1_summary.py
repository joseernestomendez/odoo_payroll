...
class TestHrReleve1SummaryBase(TestHrReleve1Base):
    def create_summary(self):
        ...
        return self.summary_model.browse(cr, uid, summary_id, context=context)
...
class TestHrReleve1Summary(TestHrReleve1SummaryBase):
    def test_compute_releve_1_summary(self):
        ...
        releve_1 = next(
            rl_1
            for rl_1 in summary.releve_1_ids
        ...
        self.assertEqual(
            ...
            round(
                releve_1.get_amount("B") + second_releve_1.get_amount("B"), 2
            ),
        )
        self.assertEqual(
            round(summary.qpip_amount_ee, 2),
            round(
                releve_1.get_amount("H") + second_releve_1.get_amount("H"), 2
            ),
        )
        self.assertEqual(
            round(summary.qit_amount_1, 2),
            round(
                releve_1.get_amount("E") + second_releve_1.get_amount("E"), 2
            ),
        )
        ...
        self.assertEqual(
            ...
            round(payslip_1["QPP_ER_C"] * 2 + payslip_2["QPP_ER_C"], 2),
        )
        self.assertEqual(
            round(summary.qpip_amount_er, 2),
            round(payslip_1["PPIP_ER_C"] * 2 + payslip_2["PPIP_ER_C"], 2),
        )

        self.assertEqual(
            round(summary.hsf_salaries, 2),
            round(payslip_1["HSF_EE_S"] * 2 + payslip_2["HSF_EE_S"], 2),
        )
