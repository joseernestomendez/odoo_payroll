...
class TestQuebecPayrollStructure(TestPayrollStructureBase):
    def setUp(self):
        ...
        self.structure_id = self.structure_model.search(
            cr, uid, [("code", "=", "QC")], context=context
        )[0]

        self.benefit_rate_ids = [
            self.benefit_rate_model.create(
                cr,
                uid,
                {
                    "name": "Test",
        ...
        self.benefit_line_ids = [
            self.benefit_rate_line_model.create(
                cr,
                uid,
                {
                    "parent_id": line[0],
    ...
    def test_create_payslip_low_wage(self):
        ...
        self.assertEqual(
            ...
            round(
                payslip["EI_EE_C"] + payslip["QPP_EE_C"] + payslip["PPIP_EE_C"]
            ),
        )
        ...
        self.assertEqual(
            round(fit_t1), round(payslip["FIT_T3"] - 0.165 * payslip["FIT_T3"])
        )
    ...
    def test_create_payslip_higher_wage(self):
        ...
        # Check the federal tax rules overriden by quebec structure
        self.assertEqual(
            round(payslip["FIT_K2"]), round(743.58 + 385.71 + 2535.75)
        )
    ...
    def test_create_payslip_end_of_year(self):
        ...
        self.assertEqual(
            round(payslip2["EI_EE_C"]), round(taxable_earning * 0.0153)
        )
        ...
        self.assertEqual(
            round(payslip2["PPIP_EE_C"]), round(taxable_earning * 0.00559)
        )

        self.assertEqual(payslip4["PPIP_ER_C"], 0)
        self.assertEqual(
            round(payslip2["PPIP_ER_C"]), round(taxable_earning * 0.00782)
        )
