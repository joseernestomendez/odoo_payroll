...
class TestHrReleve1Base(TestHrCraT4Base):
    def setUp(self):
        ...
        self.fy_id = self.fy_model.create(
            cr,
            uid,
            {
        ...
        self.contract_model.write(
            cr, uid, [self.contract_id], {"struct_id": self.structure_id}
        )
    ...
    def check_releve_1_values(self):
        ...
        self.assertEqual(
            round(slip.get_amount("A"), 2), round(payslip_1["QIT_G"] * 2, 2)
        )
        self.assertEqual(
            round(slip.get_amount("B"), 2), round(payslip_1["QPP_EE_C"] * 2, 2)
        )
        self.assertEqual(
            round(slip.get_amount("C"), 2), round(payslip_1["EI_EE_C"] * 2, 2)
        )
        self.assertEqual(
            round(slip.get_amount("D"), 2), round(payslip_1["RPP_EE_C"] * 2, 2)
        )
        self.assertEqual(
            round(slip.get_amount("E"), 2), round(payslip_1["QIT_A"] * 2, 2)
        )
        self.assertEqual(
...
class TestHrReleve1(TestHrReleve1Base):
    ...
    def test_02_check_other_info_same_source(self):
        ...
        box_id = self.box_model.search(
            cr, uid, [("code", "=", "A-1")], context=context
        )[0]
