...
class HrFiscalYear(models.Model):
    ...
    def _default_date_start(self):
        today = datetime.now()
        return datetime(today.year, 1, 1).strftime(DEFAULT_SERVER_DATE_FORMAT)
    ...
    def onchange_schedule(self):
        if self.schedule_pay and self.date_start:
            ...
            schedule_name = next(
                (
                    s[1]
                    for s in get_schedules(self)
                    if s[0] == self.schedule_pay
                ),
                False,
            )
    ...
    def create_periods(self):
        ...
        for fy in self:
            ...
            next_year_start = datetime.strptime(
                fy.date_stop, DEFAULT_SERVER_DATE_FORMAT
    ...
    def button_confirm(self):
        for fy in self:
            if not fy.period_ids:
                raise UserError(
                    _(
                        "You must create periods before confirming "
                        "the fiscal year."
                    )
                )
        self.state = "open"
